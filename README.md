...
- Drop raw PNGs into the top-level `photos/` folder and run `npm run compress:photos` to optimize them (a temporary backup lives under `photos/__original_backup` while each file is recompressed and written back as `*_processed.png`).
