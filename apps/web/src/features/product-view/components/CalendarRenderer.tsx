...
import { ConfettiSystem } from '../../../features/advent/utils/ConfettiSystem';
import { SoundManager } from '../../../features/advent/utils/SoundManager';
import { adventMemories } from '../../../data/adventMemories';
import { ProductRendererProps, ProductContent } from '../types/product';
import { ContentGrid } from './navigation/ContentGrid';
...
// Map theme system themes to calendar component themes
const mapThemeToCalendar = (theme?: string): CalendarTheme => {
  switch (theme) {
    case 'snow':
      return 'christmas';
    case 'warm':
...
/**
 * CalendarRenderer - Specialized renderer for advent calendar products
 *
 * This component adapts the existing AdventCalendar functionality to work
 * with the generalized ProductContent structure while maintaining all
 * existing calendar-specific features and behaviors.
 */
export const CalendarRenderer: React.FC<ProductRendererProps> = ({
  product,
  content,
  onContentOpen,
  theme
  ...
  // Determine if a content item should be unlocked (calendar-specific logic)
  const isContentUnlocked = (contentNumber: number) => {
    if (!isDecember) return false;
    if (currentDay >= 25) return contentNumber <= 24; // All days unlocked after Dec 25
    return contentNumber <= currentDay;
  };

  const handleButtonClick = (contentItem: ProductContent) => (event: React.MouseEvent) => {
    ...
    if (!isContentUnlocked(contentItem.content_number)) return;
  ...
  return (
    <div className={`p-4 min-h-screen ${getThemeBackground()}`}>
      {isAnimating && <div>Butterfly animation placeholder</div>}
      {selectedContent && (
        <Modal
          isOpen={isModalOpen}
          onClose={closeModal}
          ...
          photo={selectedContent.media_assets?.photo || `https://picsum.photos/400/300?random=${selectedContent.content_number}`}
          theme={calendarTheme}
          voiceUrl={selectedContent.media_assets?.voice}
          musicUrl={selectedContent.media_assets?.music}
          confettiType={selectedContent.metadata?.confetti_type}
        />
      )}
      ...
      <h1 className="text-4xl font-bold text-center text-white mb-8">
        {product.title || 'Christmas Advent Calendar'}
      </h1>

      {/* Progress Indicator */}
      <div className="mb-8">
        <ProgressIndicator
          current={content.filter(item => item.is_completed).length}
      ...
      {/* Calendar Grid using ContentGrid component */}
      <ContentGrid
        content={content}
        columns={5}
        theme={theme}
        renderItem={(contentItem, index) => {
          const isUnlocked = isContentUnlocked(contentItem.content_number);
          ...
          return (
            <motion.button
              key={contentItem.id}
              disabled={isLocked}
              onClick={handleButtonClick(contentItem)}
              ...
              transition={{
                duration: 0.3,
                delay: contentItem.content_number * 0.05
              }}
            >
              {isLocked && (
                <motion.div
              ...
              {isUnlocked && (
                <motion.span
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  transition={{ type: "spring", stiffness: 300, delay: contentItem.content_number * 0.1 }}
                >
                  {contentItem.content_number}
                </motion.span>
              )}
            </motion.button>
          );
