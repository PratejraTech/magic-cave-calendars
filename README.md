...
- Drop photo assets into the top-level `photos/` folder alongside a markdown file with the same base name (first line = button text, rest = modal copy), then run `npm run generate:photos`. The script copies each image/markdown pair into `public/photos` and regenerates `src/data/photoPairs.generated.ts`, which maps each modal to its corresponding files.
